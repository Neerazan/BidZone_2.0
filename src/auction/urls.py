...
customer_router = routers.NestedDefaultRouter(router,
                                              'customers',
                                              lookup='customer')
customer_router.register('reviews', ReviewViewSet, basename='customer-reviews')

customer_router.register('deliveries',
                         DeliveryViewSet,
                         basename='customer-deliveries')

customer_router.register('products',
                         ProductViewSet,
                         basename='customer-products')

customer_router.register('customer_coins',
                         CustomerCoinViewSet,
                         basename='customer-coins')

customer_router.register('addresses',
                         AddressViewSet,
                         basename='customer-addresses')

customer_router.register('transactions',
                         TransactionViewSet,
                         basename='customer-transactions')

products_router = routers.NestedDefaultRouter(customer_router,
                                              'products',
                                              lookup='product')
...
wishlists_router = routers.NestedDefaultRouter(router,
                                               'wishlists',
                                               lookup='wishlist')
wishlists_router.register('items', WishlistItemViewSet, basename='cart-items')

auction_router = routers.NestedDefaultRouter(router,
                                             'auctions',
                                             lookup='auction')
...
auction_router.register('questions',
                        AuctionQuestionViewSet,
                        basename='auction-questions')

answer_router = routers.NestedDefaultRouter(auction_router,
                                            'questions',
                                            lookup='question')

answer_router.register('answers',
                       AuctionAnswerViewSet,
