...
def process_completed_auctions():
    ...
    completed_auctions = Auction.objects.filter(
        ending_time__range=(time_range_start, time_range_end),
        auction_status=Auction.AUCTION_ACTIVE)
    print(f'Completed auctions: {completed_auctions}')

    for auction in completed_auctions:
        with transaction.atomic():
            highest_bid = Bid.objects.filter(
                auction=auction, status=True).order_by('-amount').first()
            ...
            if highest_bid:
                ...
                Delivery.objects.create(
                    auction=auction,
                    customer=highest_bid.bidder,
                    status=Delivery.DELIVERY_STATUS_PENDING)

                other_bids = Bid.objects.filter(
                    auction=auction, status=True).exclude(id=highest_bid.id)
                ...
                for bid in other_bids:
                    ...
                    Transaction.objects.create(
                        invoice=  # noqa: E251
                        f'Refunded for the auction {auction.product.title}',
                        user=bid.bidder,
                        amount=bid.amount,
                ...
                seller_coin, created = UserCoin.objects.get_or_create(
                    customer=seller.id)
                ...
                Transaction.objects.create(
                    invoice=  # noqa: E251
                    f'Received {highest_bid.amount} for the auction {auction.product.title}',
                    user=seller,
